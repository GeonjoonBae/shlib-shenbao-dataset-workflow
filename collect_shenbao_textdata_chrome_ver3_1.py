...
OUTPUT_BASENAME = "shenbao_textdata"
OUTPUT_FILENAME_HELP = f"{OUTPUT_BASENAME}_{{label}}_{{start_index}}to{{end_index}}.csv"
DETAIL_WRAPPER_SELECTOR = "#content-box_contentwrapper"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f"Output directory for {OUTPUT_FILENAME_HELP} files. Defaults to ./shenbao/shenbao_textdata beside this script.",
    )
    parser.add_argument(
        "--label",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=4.0,
        help="Kept for command-line compatibility. Version 2 parses detail fields after the detail wrapper is detected.",
    )
    parser.add_argument(
        "--save-every",
        type=int,
    ...
    parser.add_argument(
        "--resume-file",
        type=Path,
        help=f"Resume from a previously saved {OUTPUT_FILENAME_HELP} file.",
    )
    parser.add_argument(
        "--resume-latest",
        action="store_true",
        help=f"Resume from the latest {OUTPUT_FILENAME_HELP} file found in the output directory.",
    )
...
def read_list_entry(block: Locator) -> dict[str, str]:
    ...
    return {"list_title": title}


def empty_detail_fields() -> dict[str, str]:
    return {
        "publish_variant": "",
        "date": "",
        "issue_page": "",
...
def extract_detail_fields(scope: Page | Frame) -> dict[str, str]:
    wrapper = scope.locator(DETAIL_WRAPPER_SELECTOR).first
    if not wrapper.count():
        raise RuntimeError("Detail wrapper not found.")

    fields = wrapper.evaluate(
        """
        (root) => {
            ...
            const directText = (el) => {
                ...
                clone.querySelectorAll('div').forEach((child) => child.remove());
                return normalize(clone.textContent || '');
            };
            const extractBetweenLabels = (text, label, nextLabels) => {
                const start = text.indexOf(label);
                if (start < 0) return '';
                let value = text.slice(start + label.length);
                for (const nextLabel of nextLabels) {
                    const idx = value.indexOf(nextLabel);
                    if (idx >= 0) {
            ...
            const publishVariant = normalize(leftTitle?.textContent || '');

            let headerSpan = leftTitle?.nextElementSibling || null;
            while (headerSpan && headerSpan.tagName.toLowerCase() !== 'span') {
                headerSpan = headerSpan.nextElementSibling;
            }
            const headerText = normalize(headerSpan?.textContent || '');
            const dateMatch = headerText.match(/日期：\\s*([0-9]{4}-[0-9]{2}-[0-9]{2})/);
            const issueMatch = headerText.match(/版次\\/卷期：\\s*([^專欄]+?)(?:\\s*專欄：|$)/);
            const specialMatch = headerText.match(/專欄：\\s*(.*)$/);
            const issuePage = normalize((issueMatch?.[1] || '').replace(/版\\s*$/, ''));

            const h1El = root.querySelector(':scope > h1');
            const h1 = normalize(h1El?.textContent || '');
            ...
            const contentEl = root.querySelector('.content-box2');
            const contentBox2 = normalize(contentEl?.textContent || '');

            let metadataText = normalize(contentEl?.nextElementSibling?.textContent || '');
            if (!metadataText.includes('其他紀元：')) {
                const metadataCandidate = Array.from(root.querySelectorAll('div'))
                    .find((el) => normalize(el.textContent || '').includes('其他紀元：'));
                metadataText = normalize(metadataCandidate?.textContent || '');
            }
            ...
            return {
                ...
                special_column: normalize(specialMatch?.[1] || ''),
                h1,
                lv1_div: lv1Div,
                content_box2: contentBox2,
                era_year: extractBetweenLabels(metadataText, '其他紀元：', ['類別：', '主題：']),
                category: extractBetweenLabels(metadataText, '類別：', ['主題：']),
                theme: extractBetweenLabels(metadataText, '主題：', []),
            };
        }
        """
    )
    ...
    return {
        "publish_variant": normalize_text(fields.get("publish_variant", "")),
        "date": normalize_text(fields.get("date", "")),
        "issue_page": normalize_text(fields.get("issue_page", "")),
        "special_column": normalize_text(fields.get("special_column", "")),
        "h1": normalize_text(fields.get("h1", "")),
        "lv1_div": normalize_text(fields.get("lv1_div", "")),
        "content-box2": normalize_text(fields.get("content_box2", "")),
        "era_year": normalize_text(fields.get("era_year", "")),
        "category": normalize_text(fields.get("category", "")),
        "theme": normalize_text(fields.get("theme", "")),
    }
...
def find_detail_scope(context) -> Optional[ActiveScope]:
    for active in iter_scopes(context):
        try:
            if active.scope.locator(DETAIL_WRAPPER_SELECTOR).count():
                return active
        except PlaywrightError:
            continue
    return None
...
def build_output_path(output_dir: Path, rows: list[dict[str, str]], label: Optional[str]) -> Optional[Path]:
    ...
    if label:
        return output_dir / f"{OUTPUT_BASENAME}_{label}_{start_index}to{end_index}.csv"
    return output_dir / f"{OUTPUT_BASENAME}_{start_index}to{end_index}.csv"
...
def find_latest_resume_file(output_dir: Path, label: Optional[str] = None) -> Optional[Path]:
    pattern = (
        f"{OUTPUT_BASENAME}_{label}_*to*.csv"
        if label
        else f"{OUTPUT_BASENAME}_*to*.csv"
    )
    ...
    def parse_end_index(path: Path) -> int:
        match = re.search(r"to(\d+)$", path.stem)
...
def infer_label_from_path(path: Path) -> Optional[str]:
    match = re.match(
        rf"^{re.escape(OUTPUT_BASENAME)}_(.+)_(\d+)to(\d+)$",
        path.stem,
    )
    if not match:
        return None
...
def load_resume_rows(path: Path) -> list[dict[str, str]]:
    ...
    with path.open("r", encoding="utf-8", newline="") as f:
        ...
        for row in reader:
            ...
            restored_row = {field: row.get(field, "") for field in FIELDNAMES}
            restored_row["page"] = str(page)
            restored_row["item_index"] = str(item_index)
            restored_row["global_item_index"] = str(global_item_index)
        ...
        while True:
            ...
            for item_idx in range(start_item_idx, item_count):
                ...
                try:
                    ...
                    detail_fields = extract_detail_fields(detail_active.scope)
                except SessionExpiredError as exc:
                    failure_message = f"{type(exc).__name__}: {normalize_text(str(exc))}"
                    failure_message = build_failure_text(failure_message, detail_url, "")
                    stop_due_to_auth = True
                except Exception as exc:
                    scope_url = get_scope_url(detail_active.scope) if detail_active is not None else ""
                    failure_message = f"{type(exc).__name__}: {normalize_text(str(exc))}"
                    failure_message = build_failure_text(failure_message, detail_url, scope_url)
